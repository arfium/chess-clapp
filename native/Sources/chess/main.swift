...
// No TCP port is ever opened. See reference/app-developer.md.

let argv = Array(CommandLine.arguments.dropFirst())
let first = argv.first ?? "help"

// `<cli> --help` is the agent's ONLY manual (reference/app-developer.md § Step 3):
// document every verb here. If a verb isn't here, the agent doesn't know it exists.
let helpText = """
...
func printState(_ st: StateDTO) {
    ...
    if let note = st.note, !note.isEmpty { print("note: \(note)") }
}
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ note: Notification) {
        ...
        if let pipe = ControlPipe(signals: AppInfo.signals) {
            ...
            if pipe.start() {
                control = pipe
                state.onSignal = { [weak self] name, target, payload in
    ...
    func handle(_ req: Request) -> Response {
        ...
        case "state", "board", "fen":
            return Response(ok: true, state: state.snapshot())
        case "new":
            state.newGame(humanColor: parseColor(req.color), agent: req.agent)
            return Response(ok: true, state: state.snapshot())
        case "move":
            ...
            do {
                try state.move(from: f, to: t, promo: promo, by: .agent, agent: req.agent)
                return Response(ok: true, state: state.snapshot())
            } catch { return Response(ok: false, error: "\(error)") }
        case "resign":
            do {
                try state.resign(state.agentColor, by: .agent, agent: req.agent)
                return Response(ok: true, state: state.snapshot())
            } catch { return Response(ok: false, error: "\(error)") }
        case "takeback":
            state.takeback(req.n ?? 1)
            return Response(ok: true, state: state.snapshot())
        case "legal":
            return Response(ok: true, legal: state.legalList(from: req.square.flatMap(parseSquare)))
        case "say":
            state.setNote(req.text ?? "", agent: req.agent)
            return Response(ok: true, state: state.snapshot())
        case "help", "catalog":
...
func runClient(_ argv: [String]) -> Never {
    ...
    switch cmd {
    case "board", "state", "fen":
        req.cmd = cmd == "board" ? "state" : cmd
    case "new":
        req.color = argv.count > 1 ? argv[1] : "white"
    case "move":
        guard argv.count > 1 else { fail("usage: \(AppInfo.cli) move <uci>  e.g. e2e4") }
    ...
    case "resign":
        guard argv.count == 1 else { fail("usage: \(AppInfo.cli) resign") }
    case "takeback":
        req.n = argv.count > 1 ? Int(argv[1]) : 1
    case "focus":
        guard argv.count == 1 else { fail("usage: \(AppInfo.cli) focus") }
    case "close":
        guard argv.count == 1 else { fail("usage: \(AppInfo.cli) close") }
        req.cmd = "quit"
    default:
        fail("unknown command: \(cmd) (try: \(AppInfo.cli) help)")
    }

    // Forward our identity: Clatch injects CLATCH_AGENT into an agent's CLI shell,
    // so the running app learns who invoked it (targeting; reference/signals.md).
